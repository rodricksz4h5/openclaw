  ...
  it("disposes after the active-delivery stop grace expires", async () => {
    await withQueue(async (queue) => {
      ...
      try {
        ...
        const releaseSpy = vi.spyOn(queue, "release").mockImplementation((...args) => {
          const result = release(...args);
          void result.then(released.resolve, released.reject);
          return result;
        });
        try {
        ...
        const restartedDeliver = vi.fn(async (_event, _destination, control) => {
          try {
            ...
            replayCompleted.reject(error);
            throw error;
          }
        });
        restarted = createLineWebhookSpool({
          accountId: "default",
          runtime: runtime(),
          queue,
          deliver: restartedDeliver,
