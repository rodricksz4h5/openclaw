...
- `web.login.start` starts a QR/web login flow. Params include optional `{ channel, accountId, force, timeoutMs, verbose }`. When `channel` is present, the Gateway normalizes its canonical id or alias and dispatches only to that installed channel plugin. Omitting `channel` preserves the legacy behavior of selecting the first loaded QR-capable provider. A provider may return an opaque `sessionKey` with its QR response. If the selected plugin implements `gateway.loginWithQrStartPreflight`, the Gateway calls it before checking or stopping the channel. A non-null result is returned directly; `null` continues to `gateway.loginWithQrStart`. The preflight must not start login or include a `qrDataUrl` field; the Gateway rejects QR-bearing results. See [QR login preflight for channel plugins](/plugins/sdk-channel-plugins#qr-login-preflight).
