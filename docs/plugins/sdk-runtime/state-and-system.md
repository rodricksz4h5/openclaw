...
<AccordionGroup>
  ...
  <Accordion title="api.runtime.state">
    ...
    A shared ingress monitor joins settlement writes before releasing its drain's claim ownership, including when the channel owns the callback shutdown grace. Direct `createChannelIngressDrain(options)` callers retain abort-time recovery. A caller that owns a shutdown join can select `createChannelIngressDrain(options, true)`, abort its configured `abortSignal`, and await `dispose({ waitForSettlements: true })`. The overload requires that cancelled, retained owner; it joins writes already started and retires the drain in the same final check, without waiting for an uncooperative handler. A failed settlement rejects joined disposal and keeps the drain's claim ownership. The legacy drain interface and zero-argument synchronous `dispose()` remain unchanged.
